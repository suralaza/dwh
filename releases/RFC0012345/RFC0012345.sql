...
--@@DDL_BEGIN GRP_ODS_BW.KURL_IM
drop table if exists grp_ods_bw.kurl_im;
create writable  external table grp_ods_bw.ext_kurl_im (
guid text null,
...
--@@MODEL_BEGIN GRP_WRK.V_BW_METRICS
drop view if exists grp_wrk.v_bw_metrics;
create or replace  view grp_wrk.v_bw_metrics
AS
select 
guid as metric_id,
created_at,
...
--@@LOADER_BEGIN GRP_CORE.BW_METRICS LOAD_GP_CORE_BW_METRICS
...
--@@DDL_BEGIN GRP_REP.V_BW_METRICS
drop view if exists grp_rep.v_bw_metrics;
create or  replace   view grp_rep.v_bw_metrics AS
select 
metric_id,
created_at,
VALUE
